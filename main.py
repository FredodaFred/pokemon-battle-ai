...
win_rate = open(f'./data/win_rates', "a")
...
for i in range(130, 152):
    ...
    file = open(f'./data/{pk.name}_battle_data.csv', "w")
    header_row = ['turn_num', 'op_name', 'my_hp', 'op_hp', 'my_type1', 'my_type2', 'op_type1', 'op_type2', 'my_status', 'op_status', 'my_speed', 'op_speed' 'win']
    ...
    for j in range(1, 152):
        ...
        game_record = []
        trainer1 = PokemonTrainer("trainer1", team=[Pokemon(i)])
        team2 = [Pokemon(j)]
        ...
        while running:
            ...
            clock.tick(60)  # limits FPS to 20

    file.close()
    win_rate.write(f"Win Rate for {pk.name}: {win_num/151}\n")
